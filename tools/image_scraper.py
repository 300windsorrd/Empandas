...
Usage
  python image_scraper.py --readme Links.md --images-dir images --no-playwright
  python image_scraper.py --readme README.md  (auto-detects Playwright if installed)

Dependencies
  pip install requests beautifulsoup4
  # Optional (for JS-heavy pages like DoorDash/Grubhub/Yelp): 
...
IMG_EXT_WHITELIST = {".jpg", ".jpeg", ".png", ".gif", ".webp", ".avif", ".bmp", ".tiff"}
CONTENT_TYPE_TO_EXT = {
    "image/jpeg": ".jpg",
    "image/jpg": ".jpg",
    "image/png": ".png",
...
class ImageScraper:
    ...
    def _scrape_page(self, url: str, src_key: str, out_dir: Path) -> None:
        ...
        image_urls = [self._normalize_img_url(url, u) for u in image_urls]
        ...
        self._download_all(image_urls, out_dir)
        ...
        for img in soup.find_all("img"):
            ...
            if srcset and ("photo" in srcset or "bphoto" in srcset):
                for part in srcset.split(","):
                    ...
                    if cand:
                        urls.add(cand)
        # CSS backgrounds referencing photos
        for tag in soup.find_all(style=True):
            style = tag.get("style") or ""
    ...
    def _extract_with_playwright(self, url: str, src_key: str) -> List[str]:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Give time for lazy images to hydrate
    ...
    def _is_image_like(self, url: str) -> bool:
        # Accept if it has an image extension or looks like an image CDN with params
        ext = Path(urlparse(url).path).suffix.lower()
        if ext in IMG_EXT_WHITELIST:
            return True
        # Accept some known CDN patterns (webp/jpg encoded without extension)
        if any(k in url for k in ("/photo/", "bphoto", "images.", "image/upload", "media/") ):
            return True
        # OG/Twitter might be image endpoints without extension
        if "og:image" in url or "twimg" in url:
            return True
        return False
    ...
    def _download_all(self, urls: List[str], out_dir: Path) -> None:
        for idx, u in enumerate(urls, start=1):
            self._download_one(u, out_dir)
            time.sleep(self.delay_sec)

    def _download_one(self, url: str, out_dir: Path) -> None:
        try:
